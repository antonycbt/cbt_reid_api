...
def create_member_access(payload: MemberAccessCreate,current_user: User = Depends(get_current_user)):
...
def bulk_assign_member_access(payload: MemberAccessBulkCreate,current_user: User = Depends(get_current_user)):
...
def delete_all_access_groups(member_id: int = Query(..., description="ID of the member access"),current_user: User = Depends(get_current_user)):
